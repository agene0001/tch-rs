...
fn train_tiny_net(opt_of: impl FnOnce(&nn::VarStore) -> nn::Optimizer) -> (Vec<f64>, Vec<f64>) {
    ...
    tch::no_grad(|| {
        ...
        let _ = l1.ws.copy_(&((Tensor::arange(32, cpu).view([8, 4]) - 16.0) / 20.0));
        let _ = l1.bs.as_mut().unwrap().copy_(&((Tensor::arange(8, cpu) - 4.0) / 10.0));
        let _ = l2.ws.copy_(&((Tensor::arange(8, cpu).view([1, 8]) - 4.0) / 9.0));
        let _ = l2.bs.as_mut().unwrap().copy_(&Tensor::from(0.1f32));
    });
